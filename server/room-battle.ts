	...
	updateTurn() {
		...
		if (this.battle.turn <= this.turn) {
			if (this.battle.players.filter(p => !p.request.isWait).length <= 1) {
				...
				this.battle.room.addRaw(`<!-- adding ${addPerMidTurnRequest} to each for mid turn request -->`);
...
export class RoomBattle extends RoomGame<RoomBattlePlayer> {
	...
	receive(lines: string[]) {
		...
		const link = "https://play.tanoso.online/replays/" + format + "/" + id + "_" + p1Cap + "_vs_" + p2Cap + ".html";
